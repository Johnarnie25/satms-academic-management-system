...
// ================= GET STUDENT ID =================
$student_id = 0;
$resStudent = mysqli_query($conn,"SELECT student_id FROM students WHERE user_id='".intval($user_id)."' LIMIT 1");
...
<div class="head-title">
  <div class="left">
    <h1>My Quiz Grades</h1>
    <ul class="breadcrumb">
      <li><a href="#">Home</a></li>
      <li><i class='bx bx-chevron-right'></i></li>
      <li><a class="active">My Quizzes</a></li>
    </ul>
  </div>
</div>
...
<a href="?page=<?= $i ?>&limit=<?= $limit ?>&filterYear=<?= urlencode($filterYear) ?>&filterSem=<?= urlencode($filterSem) ?>&filterSubject=<?= urlencode($filterSubject) ?>&search=<?= urlencode($search) ?>" class="btn small <?= $i==$page?'primary':'' ?>"><?= $i ?></a>
<?php } ?>
