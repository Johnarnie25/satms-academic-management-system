...
$res = mysqli_query($conn,"
    SELECT s.subject_id, s.subject_name, s.semester, s.year, s.schedule_days, s.schedule_time
    FROM student_subjects ss
    JOIN subjects s ON ss.subject_id=s.subject_id
    $where
...
<div class="head-title">
  <div class="left">
    <h1>Student Subject Schedule</h1>
    <ul class="breadcrumb">
      <li><a href="#">Home</a></li>
      <li><i class='bx bx-chevron-right'></i></li>
      <li><a class="active">Subjects</a></li>
    </ul>
  </div>
</div>
...
<div class="mt-15">
<?php if($page>1){ ?>
<a href="?student_id=<?= $student_id ?>&page=<?= $page-1 ?>&limit=<?= $limit ?>&filterYear=<?= $filterYear ?>&filterSem=<?= $filterSem ?>&search=<?= urlencode($search) ?>" class="btn small">Prev</a>
<?php } ?>
<?php for($i=1;$i<=$totalPages;$i++){ ?>
<a href="?student_id=<?= $student_id ?>&page=<?= $i ?>&limit=<?= $limit ?>&filterYear=<?= $filterYear ?>&filterSem=<?= $filterSem ?>&search=<?= urlencode($search) ?>" class="btn small <?= $i==$page?'primary':'' ?>"><?= $i ?></a>
<?php } ?>
<?php if($page<$totalPages){ ?>
<a href="?student_id=<?= $student_id ?>&page=<?= $page+1 ?>&limit=<?= $limit ?>&filterYear=<?= $filterYear ?>&filterSem=<?= $filterSem ?>&search=<?= urlencode($search) ?>" class="btn small">Next</a>
<?php } ?>
</div>
<?php } ?>
