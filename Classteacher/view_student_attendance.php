...
if($teacher_id==0){
    die("<div class='alert alert-danger'>Teacher access only.</div>");
}

// ================= FETCH STUDENTS =================
$students = [];
$q = mysqli_query($conn,"
    SELECT DISTINCT st.student_id,u.first_name,u.last_name
...
<?php foreach($students as $s){ ?>
    <div class="student-item">
        <input type="radio" name="student_radio" value="<?= $s['student_id'] ?>" id="student_<?= $s['student_id'] ?>" <?= ($student_id==$s['student_id']?'checked':'') ?>>
        <label for="student_<?= $s['student_id'] ?>"><?= htmlspecialchars($s['last_name'].", ".$s['first_name']) ?></label>
    </div>
<?php } ?>
</div>
...
function renderStudentList(list, page=1, limit=10){
    ...
    if(totalPages > 1){
        if(page > 1){ 
            ...
            btn.addEventListener('click', () => renderStudentList(list, page-1, limit)); 
            studentPager.appendChild(btn); 
        }
        for(let i=1;i<=totalPages;i++){
            const btn = document.createElement('button'); 
            btn.className = 'btn small'+(i===page?' primary':''); 
            btn.style.marginLeft='6px'; 
            btn.textContent = i; 
            btn.addEventListener('click', ((p)=>()=>renderStudentList(list,p,limit))(i)); 
            studentPager.appendChild(btn);
        }
        if(page < totalPages){ 
            const btn = document.createElement('button'); 
    ...
    slice.forEach(item => {
        ...
        if(selectedStudentId && radio.value == selectedStudentId) radio.checked = true;

        radio.addEventListener('change', function(){
            selectedStudentId = this.value;
            let existing = document.querySelector('input[name="student_id"]');
            if(!existing){
                existing = document.createElement('input');
                existing.type = 'hidden';
                existing.name = 'student_id';
                document.querySelector('form').appendChild(existing);
            }
            existing.value = selectedStudentId;
        });
...
studentSearchInput.addEventListener('keyup', () => {
    ...
    studentTimer = setTimeout(() => {
        const filtered = getFilteredStudents(studentSearchInput.value);
        renderStudentList(filtered, 1, 10);
    }, 300);
});
